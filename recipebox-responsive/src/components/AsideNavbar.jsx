  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-gray-200 bg-white px-4 py-6 shadow-sm lg:flex">
        <button onClick={() => navigate("/search")} className="mb-8 flex items-center justify-center">
          <img src={logo} alt="RecipeBox logo" className="h-24 w-24 object-contain" />
        </button>

        <nav className="space-y-2">
          {items.filter((item) => item.show).map(({ label, icon: Icon, path }) => (
            <button
              key={label}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 flex min-h-16 items-center justify-around border-t border-gray-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_18px_rgba(0,0,0,0.08)] backdrop-blur lg:hidden">
        <button onClick={() => navigate("/search")} className="flex min-w-16 flex-col items-center gap-1 py-2 text-green-950">
          <Home className="size-5" />
          <span className="text-[10px] font-semibold">Home</span>
        </button>

        {isLoggedIn && (
          <button onClick={() => navigate("/myrecipes")} className="flex min-w-16 flex-col items-center gap-1 py-2 text-green-950">
        ...
        {isLoggedIn && (
          <button className="flex min-w-16 flex-col items-center gap-1 py-2 text-green-950">
            <ShoppingBasket className="size-5" />
        ...
        {isLoggedIn && (
          <button onClick={handleLogout} className="flex min-w-16 flex-col items-center gap-1 py-2 text-red-600">
            <LogOut className="size-5" />
